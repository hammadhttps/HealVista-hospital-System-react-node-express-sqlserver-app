...
export function AppointmentQR({ qrToken, appointmentNo, size = "sm" }: AppointmentQRProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger render={<Button size={size} variant="outline" />}>
        {t("appointments:showQr")}
      </DialogTrigger>
      <DialogContent className="sm:max-w-sm">
        <DialogHeader>
          <DialogTitle>
            {t("appointments:checkInCode")}
        ...
        <div className="flex flex-col items-center gap-4 py-2">
          ...
          {error && <p className="text-sm text-destructive">{t("appointments:qrError")}</p>}

          {dataUrl && (
            <img
              src={dataUrl}
              alt={t("appointments:qrAlt", { no: appointmentNo ?? "" })}
