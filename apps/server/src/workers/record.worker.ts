...
import { extractRecordText } from "../services/record.service.js";
 ...
 *
 * Runs after an upload: `extractRecordText` downloads the PDF behind its
 * authenticated Cloudinary delivery URL, extracts the text with pdfjs-dist, and
 * stores it in `MedicalRecord.extractedText`. Phase 5 retrieves those chunks for
 * RAG over a patient's documents, so this is where the searchable plain text is
 * born. The same function also runs inline in `registerRecord` when Redis is off
 * and no worker could run at all.
 *
...
export function startRecordWorker(): Worker | null {
  ...
  const worker = new Worker(
    ...
    async (job) => {
      ...
      const { extracted, chars } = await extractRecordText(recordId);
      if (!extracted) {
      ...
      logger.info({ recordId, chars }, "[record-worker] Text extracted");
    },
